...
public class PairingFactory {
    ...
    public PairingParameters loadParametersString(String paramsString) {
        PropertiesParameters curveParams = new PropertiesParameters();
        InputStream is = new ByteArrayInputStream(Charset.forName("UTF-8").encode(paramsString).array());
        curveParams.load(is);
