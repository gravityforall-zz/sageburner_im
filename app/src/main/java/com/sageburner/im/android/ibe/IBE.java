...
public class IBE {
	...
	public IBE(String paramsString, String pByteString, String sByteString) {
	...
	private void setup() {
		...
		// Load the parameters...
		PairingParameters params = PairingFactory.getInstance().loadParametersString(paramsString);
		...
		try {
			...
			if (pByteString != null && pByteString.length() > 0 && sByteString != null && sByteString.length() > 0) {
				byte[] PbyteIn = this.toByteArray(pByteString);
				// System.out.println(Pstr);
				P.setFromBytes(PbyteIn);
				byte[] SbyteIn = this.toByteArray(sByteString);
				s.setFromBytes(SbyteIn);
			} else {
				Log.d("IBE::setup: ", "Initializing parameters not found!");
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
